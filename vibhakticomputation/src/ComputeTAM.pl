...
=cut
		...
		#print "final_tam_aux : ",$final_tam_aux,"\n";
...
=cut	#print "fs : ",$fs_head_array->[0],"\n";
		foreach my $hashRef ($fs_head_array->[0]) {
				    foreach my $key (keys %{$hashRef}) {
				        my $value = $hashRef->{$key};
				        print "$key => @$value\n";
				    }
				    print "---\n";  # Separating each hash with '---' for clarity
				}
=cut
